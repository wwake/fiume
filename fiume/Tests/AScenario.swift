...
struct AScenario {
  ...
  private func makeLeia(
    ...
    Leia(name: name, amount: .money(amount), dates: DateRange(.month(first), .month(last)), type: .income)
  }
  ...
  private func makeLeia(
    ...
    Leia(name: name, amount: .money(amount), dates: DateRange(first, last), type: leiaType)
  }
