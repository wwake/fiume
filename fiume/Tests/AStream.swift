...
final class AStream: XCTestCase {
  ...
  func test_determines_amount_inside_date_range() throws {
    ...
    XCTAssertEqual(sut.amount(for: MonthYear(month: 1, year: 2020), month: 1), Money(100))
    XCTAssertEqual(sut.amount(for: MonthYear(month: 1, year: 2020), month: 10), Money(100))
  }

  func test_determines_amount_outside_date_range() throws {
    let sut = makeStream(100, first: 1, last: 10)
    XCTAssertEqual(sut.amount(for: MonthYear(month: 1, year: 2020), month: 0), Money(0))
    XCTAssertEqual(sut.amount(for: MonthYear(month: 1, year: 2020), month: 12), Money(0))
  }
